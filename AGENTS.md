# AI Repository Index — PHIL 201

## Purpose

This public repository is the course archive for PHIL 201. Class Scribe adds lecture notes after a recording is transcribed and passes automated quality checks. Use this file as the entry point before answering questions from the repository.
...
Enumerate them dynamically with `rg --files notes`, keep only those dated patterns, and sort by path. Do not rely on a hand-maintained lecture list: new files arrive automatically. PHIL 201 normally has two recordings per class week. A date may be one day earlier than the scheduled class day because the recording app has produced that convention. Other Markdown files are supplemental unless they follow the dated path convention and contain a `Transcript` section.
